...
fn main() {
    ...
    let addr = if let Some(pa) = parser.get_parsed_argument_long("addr") {
        log::info!("Trying to use supplied value for addr...");
        pa.value.unwrap_or("127.0.0.1".to_owned())
    } else {
        log::info!("Using default value 127.0.0.1 for addr...");
        "127.0.0.1".to_owned()
    };

    let port = if let Some(pa) = parser.get_parsed_argument_long("port") {
        log::info!("Trying to use supplied value for port...");
        pa.value.unwrap_or("8080".to_owned())
    } else {
        log::info!("Using default value 8080 for port...");
        "8080".to_owned()
    };

    let root = if let Some(pa) = parser.get_parsed_argument_long("root") {
        log::info!("Trying to use supplied value for root...");
        pa.value.unwrap_or(".".to_owned())
    } else {
        log::info!("Using default root folder where instance has been launched...");
        ".".to_owned()
    };

    log::info!("Starting server on {addr}:{port}, serving '{}'...", root);
    ...
    HttpServer::new(
        ...
        Some(Box::new(|request: &HttpRequest| {
            log::debug!("Got unimplemented request {:?}", request);
            HttpResponse {
    ...
    .get(
        ...
        Box::new(move |request: &HttpRequest| {
            ...
            log::debug!("Trying to handle path {}", uri_path.display());
            match rooted_path {
                ...
                _ => {
                    log::warn!("File {} not found", rooted_path.display());
                    HttpResponse::new(
                        "1.1".to_owned(),
                        HttpStatus::NotFound,
                        None,
...
pub fn handle_file(path: PathBuf, root: PathBuf) -> HttpResponse {
    ...
    let len = match file.metadata() {
        ...
        Err(err) => {
            log::warn!("Could not get metadat: {err}");
            return HttpResponse::new("1.1".to_owned(), HttpStatus::NotFound, None, None);
        }
    };

    log::debug!("Sending file {} with length {}", path.display(), len);
    HttpResponse {
...
pub fn handle_folder(path: PathBuf, root: PathBuf) -> HttpResponse {
    ...
    log::debug!("Sending {} folder entries...", entries.len());
    HttpResponse {
